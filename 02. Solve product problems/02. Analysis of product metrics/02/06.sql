...
Колонку с наименованиями кампаний назовите ads_campaign, колонку с днями — day,
а колонки со значениями метрик — cumulative_arppu и cac.
...
затем по наименованию дня (также по возрастанию).
Поля в результирующей таблице: ads_campaign, day, cumulative_arppu, cac
*/

SELECT ads_campaign, day, ROUND(cost_order / users, 2) AS cumulative_arppu, cac
FROM (SELECT ads_campaign,
             concat('Day ', EXTRACT('day'FROM date)::INTEGER - 1) AS day,
             SUM(SUM(order_price)) OVER (PARTITION BY ads_campaign ORDER BY concat('Day ', EXTRACT('day'FROM date)::INTEGER - 1)) AS cost_order
      FROM (SELECT CASE WHEN user_id IN (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732, 8739, 8741,
                        ...
                        ELSE 'Не кампания'
                        END AS ads_campaign,
                   time::DATE AS date, user_id, order_id
            FROM user_actions
            WHERE order_id NOT IN (SELECT order_id FROM user_actions WHERE action = 'cancel_order') AND DATE_PART('month', time) > 8) AS t1
      LEFT JOIN
           (SELECT order_id, SUM(price) AS order_price
            FROM (SELECT order_id, UNNEST(product_ids) AS product_id FROM orders) AS t2
      LEFT JOIN products USING (product_id)
      GROUP BY order_id) AS t3
      USING (order_id)
      WHERE ads_campaign != 'Не кампания'
      GROUP BY ads_campaign, concat('Day ', EXTRACT('day'FROM date)::INTEGER - 1)) AS t3
LEFT JOIN
     (SELECT ads_campaign, COUNT(DISTINCT user_id) AS users, ROUND(250000.0 / COUNT(DISTINCT user_id), 2) AS cac
      FROM (SELECT 'Кампания № 1' AS ads_campaign, user_id
            FROM user_actions
            WHERE order_id NOT IN (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
                  AND user_id IN (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732, 8739, 8741,
                                  ...
                                  9887, 9891, 9896, 9897, 9916, 9945, 9960, 9963, 9965, 9968, 9971, 9993, 9998,
                                  9999, 10001, 10013, 10016, 10023, 10030, 10051, 10057, 10064, 10082, 10103,
                                  10105, 10122, 10134, 10135)) AS t1
            GROUP BY ads_campaign
      UNION
      SELECT ads_campaign, COUNT(DISTINCT user_id) AS users,  ROUND(250000.0 / COUNT(DISTINCT user_id), 2) AS cac
      FROM (SELECT 'Кампания № 2' AS ads_campaign, user_id
            FROM user_actions
            WHERE order_id NOT IN (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
                  AND user_id IN (8629, 8630, 8644, 8646, 8650, 8655, 8659, 8660, 8663, 8665, 8670, 8675, 8680, 8681, 
                                  ...
                                  9877, 9879, 9880, 9882, 9883, 9885, 9901, 9904, 9908, 9910, 9912, 9920, 9929, 9930,
                                  9935, 9939, 9958, 9959, 9961, 9983, 10027, 10033, 10038, 10045, 10047, 10048, 10058,
                                  10059, 10067, 10069, 10073, 10075, 10078, 10079, 10081, 10092, 10106, 10110, 10113, 10131)) AS t2
      GROUP BY ads_campaign) AS t4
USING (ads_campaign)
