...
В результат включите четыре колонки: колонку с наименованиями кампаний, дату первого взаимодействия с
приложением, количество дней, прошедших с даты первого взаимодействия (порядковый номер), и само значение Retention.
...
FROM (SELECT ads_campaign, start_date, date, COUNT(DISTINCT user_id) AS users
      FROM (SELECT CASE WHEN user_id IN (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732, 8739, 8741,
                        ...
                        ELSE 'Не кампания'
                        END AS ads_campaign,
                   MIN(time::DATE) OVER (PARTITION BY user_id) AS start_date, time::DATE AS date, user_id
